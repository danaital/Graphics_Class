...
public class Ops {
    ...
    public static Vec refract(Vec u, Vec normal, double n1, double n2) {
        ...
        double dotsquared = Math.pow(Ops.dot(Ops.neg(u), normal), 2);
        double n1squared = Math.pow(n1, 2);
        double n2squared = Math.pow(n2, 2);
        if (n1 > n2 && (1 - dotsquared > n1squared / n2squared) ){
            Ops.reflect(u, normal);
        }

        double sinTheta2 = n1squared * (1 - dotsquared) / n2squared;
        double cosTheta2 = Math.sqrt(1 - sinTheta2);
        sinTheta2 = Math.sqrt(sinTheta2);
        Vec b = Ops.normalize(Ops.add(u, Ops.mult(Ops.dot(Ops.neg(u), normal), normal)));
