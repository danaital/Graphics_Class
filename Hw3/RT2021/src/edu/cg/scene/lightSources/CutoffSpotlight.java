...
public class CutoffSpotlight extends PointLight {
    ...
    public Vec intensity(Point hittingPoint, Ray rayToLight) {
        Vec tDirect = this.direction.normalize().neg();
        Vec oDirect = rayToLight.direction().normalize();
        double consineGamma = tDirect.dot(oDirect);
        double gamma = Math.toDegrees(Math.acos(consineGamma));
        if (consineGamma >= Ops.epsilon && gamma <= this.cutoffAngle) {
            return super.intensity(hittingPoint, rayToLight).mult(consineGamma);
        }
        return new Vec(0.0);
    }
}
