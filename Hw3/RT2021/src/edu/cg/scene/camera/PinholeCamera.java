...
public class PinholeCamera {
    ...
    public Point transform(int x, int y) {
        // calculate the distances and add to current vector
    	Vec moveRight = this.rightVec.mult(distanceCalc(this.resX, x));
        Vec moveUp = this.upVec.mult((-1) * distanceCalc(this.resY, y));
        // move the camera accordingly
        return this.centerPoint.add(moveUp).add(moveRight);
    }

    private double distanceCalc(double resAxis, int axisIndex) {
        return (axisIndex - (int) (resAxis / 2)) * this.getPixelLength();
    }
